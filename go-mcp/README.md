...
[MCP](https://modelcontextprotocol.io) server exposing the Myip SDK as
two agent tools — `myip_list` and `myip_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/myip-mcp)
make build

# 2. Provide credentials via the environment
export MYIP_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user myip \
  -- /absolute/path/to/myip-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./myip-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// myip_list: first page of records
{ "entity": "get_ip_info" }
{ "entity": "get_ip_info", "query": { } }

// myip_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/myip-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user myip \
     -- "$PWD"/dist/*/myip-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `myip_list` and `myip_load` tools now appear
   in new sessions. Ask the agent to *"list get_ip_info using myip"*
   and it calls `myip_list` with `{"entity":"get_ip_info"}`.
...
```sh
export MYIP_APIKEY=sk_live_xxx            # API key
export MYIP_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `myip_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `myip_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MYIP_APIKEY` | API key sent with every request. |
| `MYIP_BASE` | Optional override of the API base URL. |
...
```sh
./myip-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"myip_load","arguments":{"entity":"get_ip_info","query":{"id":1}}}}'
```

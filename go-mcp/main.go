// Command Myip-mcp is an MCP server exposing the Myip
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Myip_list  — list records of an entity
//	Myip_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: MYIP_APIKEY carries the API key and
	// MYIP_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MYIP_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MYIP_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMyipSDK(opts)

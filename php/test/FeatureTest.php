...
class FtCtrl extends MyipControl
{
...
class FtClient
{
    ...
    private MyipUtility $utility;

    public function __construct(MyipUtility $utility, array $options)
    {
    ...
    public function get_utility(): MyipUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MyipContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MyipContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MyipConfig::make_feature($name);
    ...
    private static function buildUrl(MyipSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MyipError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MyipResponse($fetched);
        }
    ...
    private function fail(MyipContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MyipContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

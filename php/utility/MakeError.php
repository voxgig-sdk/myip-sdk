...
class MyipMakeError
{
    public static function call(?MyipContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new MyipContext([], null);
        }
        $op = $ctx->op ?? new MyipOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new MyipResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof MyipError) ? $err->msg : (string)$err;
        $msg = "MyipSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new MyipError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof MyipError) {

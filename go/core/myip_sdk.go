...
import (
	...
	vs "github.com/voxgig-sdk/myip-sdk/go/utility/struct"
)
...
func (sdk *MyipSDK) OptionsMap() map[string]any {
...
func (sdk *MyipSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *MyipSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *MyipSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *MyipSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *MyipSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *MyipSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("MyipSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *MyipSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *MyipSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("MyipSDK: graphql: %s", msg)
		res["graphql"] = errors
	}

...
boru-driven command-line client **and** interactive REPL for the Myip
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/myip-cli)
make build
...
./myip-cli load 1 get_ip_info            # {id:1} shorthand
./myip-cli load '{id:1}' get_ip_info       # explicit match map

# 5. Override the API base URL for a single call
MYIP_BASE=https://api.example.com ./myip-cli load 1 get_ip_info

# 6. No arguments -> interactive REPL
./myip-cli
myip> load 1 get_ip_info
myip> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/myip-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/myip-cli load 1 get_ip_info
   ```
...
```sh
./myip-cli load 1 get_ip_info          # scalar shorthand for {id:1}
./myip-cli load '{id:1}' get_ip_info     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `myip>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/myip-cli`. |
...
(`./myip-cli <expr>`) and interactively (the REPL), and expressions compose the

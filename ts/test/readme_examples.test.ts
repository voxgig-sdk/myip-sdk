...
function isRunnable(code: string): boolean {
  return (
    /new\s+MyipSDK\b/.test(code) ||
    /\bMyipSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MyipSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MyipSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

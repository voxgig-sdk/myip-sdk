...
class GetIpInfoEntity
  ...
  def data_set(args)
    if args
      @_data = MyipHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = MyipHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [GetIpInfo, Hash] the loaded GetIpInfo; raises MyipError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = MyipHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end

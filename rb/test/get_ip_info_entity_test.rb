...
class GetIpInfoEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MYIP_TEST_GET_IP_INFO_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def get_ip_info_basic_setup(extra)
  ...
  entid_env_raw = ENV["MYIP_TEST_GET_IP_INFO_ENTID"]
  ...
  if env["MYIP_TEST_LIVE"] == "TRUE"
    ...
    client = MyipSDK.new(Helpers.to_map(merged_opts))
  end
